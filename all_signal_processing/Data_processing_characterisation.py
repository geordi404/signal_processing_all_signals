...
def Data_processing(directory, patient, session, Perclos_treshold, Perclos_window_size, Lane_deviation_window_size, plotting_activated):
    ...
    if dataframes:
        ...
        #plot_ecg_signals_V_2_0(directory,resampled_dfs, patient, session, plotting_activated)
    else:
...
def plot_ecg_signals_V_2_0(directory, resampled_dfs, patient, session, plotting_activated):

    def calculate_metrics(true_peaks, detected_peaks, tolerance):
    ...
    try:
        ...
        if plotting:
            ...
            for stm_key in ['Stm32ECG_0', 'Stm32ECG_1', 'Stm32ECG_2']:
                tp, fn, fp = calculate_metrics(ECG_peaks['Biopac_ECG'], ECG_peaks[stm_key], tolerance)
                tn = len(ECG_peaks['Biopac_ECG']) - tp - fn  # true negatives not directly computable here
            ...
            ##SIGNAl BIOPAC
            ax[0].set_title('Biopac ECG Signal')
            ax[0].plot(ecg_time, ECGs_filtered["Biopac_ECG"], label='Biopac_ECG')
            ax[0].scatter(ecg_time[ECG_peaks["Biopac_ECG"]], ECGs_filtered["Biopac_ECG"][ECG_peaks["Biopac_ECG"]], color='red', label='Peaks')
            ax[0].set_ylabel('Amplitude')
            ax[0].set_xlabel('Time')
            ax[0].legend(loc='lower left')  # Change legend location to bottom left

            ##SIGNAUX EXPERIMENTAUX FILTRÉS
            for index,column in enumerate(ECG_identifiants):
                if index>0: #skip biopac
                    if column in ECG_identifiants:
                        ax[1].plot(ecg_time, ECGs_filtered[column], label=column)
                        ax[1].scatter(ecg_time[ECG_peaks[column]], ECGs_filtered[column][ECG_peaks[column]],color='red', label=f'{column} Peaks')
            ax[1].set_title('Processed STM32 ECG Signals')
            ax[1].set_ylabel('Amplitude')
            ax[1].set_xlabel('Time')
            ax[1].legend(loc='lower left')  # Change legend location to bottom left
            ##SIGNAUX EXPERIMENTAUX non filtrés
            for index,column in enumerate(ECG_identifiants):
                if index>0: #skip biopac
                    if column in ECG_identifiants:
                        ax[2].plot(ecg_time, ECGs_non_filtered[column], label=column)
                        ax[2].scatter(ecg_time[ECG_peaks[column]], ECGs_non_filtered[column][ECG_peaks[column]],color='red', label=f'{column} Peaks')
            ax[2].set_title('Processed STM32 ECG Signals')
            ax[2].set_ylabel('Amplitude')
            ax[2].set_xlabel('Time')
            ax[2].legend(loc='lower left')  # Change legend location to bottom left
